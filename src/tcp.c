...
/**
 * @brief tcp_reset 复位通知
 * 
 * @param 
*/
void tcp_reset(tcp_connect_t * connect, uint32_t seq_num) {
    printf("!!! reset tcp !!!\n");
    connect->next_seq = 0;
...
void tcp_in(buf_t* buf, uint8_t* src_ip) {
    ...
    */
    ...
    case TCP_ESTABLISHED:
       ...
       // TODO
        if (flags->ack && connect->unack_seq < seq_num && connect->next_seq > seq_num)
        {
            // buf_remove_header(buf, sizeof(tcp_hdr_t));
            // 应该是去掉connect中tx_buf中的数据
            int to_remove = connect->next_seq - connect->unack_seq;
